...
- **Pre-filtering:** restrict to eligible items, then search. Correct, but expensive if the eligible set is large and not indexed for search.
...
This service was previously branded Vertex AI Vector Search; its Python SDK classes still use `MatchingEngine` names. Semantics verified against Google Cloud documentation on 23 September 2026.[^1][^2][^3]
...
Add a datapoint that denies the token `refurbished` in its own restricts. Predict which queries it can and cannot match, then check your answer against the deny-list rules in the documentation.[^1]
