...
Here $N$ is the document count and $\operatorname{df}(t)$ counts documents containing term $t$.

Term frequency can be a raw count, a count divided by document length, or a sublinear transform.

State which one is used. [^1][^3]
...
**Inputs:** `boots` appears twice in a 10-token document and in 10 of 100 documents.

**1. Calculate normalised TF.**

$$
TF=\frac{2}{10}=0.2
$$

**2. Calculate IDF using natural logarithms.**

$$
IDF=\ln\left(\frac{100}{10}\right)=\ln10
$$

**3. Multiply the two factors.**

$$
TF\text{-}IDF=0.2\ln10\approx0.4605
$$

A term in all 100 documents has zero IDF under this particular formula.
...
### Compare a repeated rare term with a common term

Use a collection of $N=100$ documents and raw-count TF.

**Rare term:** `boots` occurs in 10 documents and twice in D1.

$$
w_{\mathrm{boots},D1}=2\ln\left(\frac{100}{10}\right)\approx4.6052
$$

**Common term:** `red` occurs in 50 documents and once in D1.

$$
w_{\mathrm{red},D1}=\ln\left(\frac{100}{50}\right)\approx0.6931
$$
...
An absent vocabulary term needs an explicit handling rule.

The unsmoothed formula is undefined at $df=0$, but most fixed-vocabulary vectorizers simply do not create a coordinate for an unseen term.

Inspect this boundary when a query becomes an all-zero vector.
