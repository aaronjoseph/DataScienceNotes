...
Let $g(t)$ be the normaliser applied to a token. Two spellings can match through normalisation when $g(t_1)=g(t_2)$. The spelling of the resulting stem is less important than which words it groups together.[^1] However, once distinct meanings share the same indexed value, that value alone cannot recover the original distinction.
...
> [!example]- Design a controlled comparison
